...
- `threshold`: Threshold for detecting changes (default: 0.0)
...
# Plot
recent_data <- data[180:200]
plot_df <- rbind(
  data.frame(time = 1:21, value = recent_data, type = "Observed"),
  data.frame(time = 22:41, value = fc$mean, type = "Forecast")
)

ggplot(plot_df, aes(x = time, y = value, color = type)) +
  geom_line(linewidth = 1) +
  scale_color_manual(values = c("Observed" = "black", "Forecast" = "red")) +
  labs(title = "ARMA Model: AR(1) Process Forecast", x = "Time", y = "Value", color = NULL) +
  theme_minimal() +
  theme(legend.position = "top")
```
