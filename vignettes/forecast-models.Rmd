...
forecastbaselines provides 10 baseline forecasting models across three
categories:
...
| STL | Medium | 14+ cycles | Yes | Yes | Via residuals |
| ARMA | Medium | 50+ | Yes | Yes | Parametric |
...
ggplot(plot_df, aes(x = time, y = value)) +
  ...
  geom_hline(
    yintercept = data[100], linetype = "dashed", color = "red", alpha = 0.5
  ) +
  scale_color_manual(values = c("Observed" = "black", "Forecast" = "red")) +
  labs(
    title = "Constant Model: Random Walk Forecast", x = "Time", y = "Value",
    color = NULL
  ) +
  theme_minimal() +
...
fc_marginal <- forecast(
  marginal, interval_method = NoInterval(), horizon = 1:100
)
...
ggplot(density_df, aes(x = x, y = y, color = type)) +
  ...
  labs(
    title = "Marginal vs KDE", x = "Forecast Value", y = "Density",
    color = NULL
  ) +
  theme_minimal() +
...
2. Average every one of them, along with the `window_width` time points
   either side of each
...
- `window_width`: Number of neighbouring time points averaged either side of
  each seasonal position (default: 0; must be smaller than `s`)
...
# Fit LSD model
# Average each month with the 3 time points either side
model <- LSDModel(window_width = 3, s = 12)
fitted <- fit_baseline(data, model)
...
ggplot(plot_df, aes(x = time, y = value, color = type)) +
  ...
  scale_color_manual(values = c("Observed" = "black", "Forecast" = "red")) +
  labs(
    title = "LSD Model: Seasonal Forecast", x = "Time", y = "Value",
    color = NULL
  ) +
  theme_minimal() +
  theme(legend.position = "top")
...
- `window_width` (neighbouring points averaged in):
  - 0: Each seasonal position on its own, keeping the sharpest profile
  - 1-2: Mild smoothing across adjacent positions
  - Larger: Blurs the seasonal profile towards a flat mean, and values of `s`
...
fc_quadratic <- forecast(
  quadratic, interval_method = NoInterval(), horizon = 1:10
)
...
ggplot(plot_df, aes(x = time, y = value, color = type)) +
  ...
  scale_linewidth_manual(
    values = c("Data" = 0.5, "Linear" = 1, "Quadratic" = 1)
  ) +
  scale_color_manual(
    values = c("Data" = "black", "Linear" = "blue", "Quadratic" = "red")
  ) +
  labs(
    title = "OLS: Linear vs Quadratic Trend", x = "Time", y = "Value",
...
Description:
Detects trend direction (increasing, decreasing, stable) across the most
recent observations and extrapolates.

How It Works:

1. Take the last `window_size` observations
2. Classify the signs of their successive differences as increasing,
...
- `window_size`: Number of recent observations to analyse (default: 3)
...
# Fit IDS model
model <- IDSModel(window_size = 3) # Analyse the last 3 observations
fitted <- fit_baseline(data, model)
...
ggplot(plot_df, aes(x = time, y = value, color = type)) +
  ...
  scale_color_manual(values = c("Observed" = "black", "Forecast" = "red")) +
  labs(
    title = "IDS Model: Trend Detection", x = "Time", y = "Value",
    color = NULL
  ) +
  theme_minimal() +
  theme(legend.position = "top")
...
- `window_size` (observations analysed):
...
- ❌ 13 seasonal cycles or fewer, which the fit rejects
...
ggplot(plot_df, aes(x = time, y = value, color = type)) +
  ...
  scale_color_manual(values = c("Observed" = "black", "Forecast" = "red")) +
  labs(
    title = "STL Model: Trend + Seasonality", x = "Time", y = "Value",
    color = NULL
  ) +
  theme_minimal() +
  theme(legend.position = "top")
...
- ✅ Smooth seasonality, through `s` and one harmonic wave
- ❌ Strong trends (use differencing, detrend first, or set `include_drift`)
- ❌ Sharp or asymmetric seasonal peaks, which need several waves (see `k`)

Parameters:

- `p`: Autoregressive order
- `q`: Moving average order
- `s`: Seasonal period (0 for no seasonality)
- `k`: Number of harmonic waves representing seasonality (default: 1).
  Estimation starts its mean parameters at zero and can settle on a solution
  ...
  while on low-count series they usually, though not always, succeed. Compare
  the fitted mean with the data
- `include_drift`: Add a linear trend to the mean (default: FALSE)
...
- `s`: Seasonal period (0 for no seasonality)
- `k`: Number of harmonic waves representing seasonality (default: 1)
- `nb`: Use a negative binomial instead of a Poisson distribution
  (default: FALSE)
...
- `k` (harmonic waves):
  - 1: Smooth, symmetric seasonal wave
  - Above 1: fitted correctly, but forecasts in ForecastBaselines.jl evaluate
    every harmonic at the fundamental frequency, so the estimated seasonal
    shape is not carried into the forecasts

- `nb` (negative binomial):
  - Set to `TRUE` when counts are overdispersed relative to a Poisson
...
- `trend_type`: "N", "A", "M", "Ad" (damped additive) or "Md" (damped
  multiplicative)
- `season_type`: "N", "A" or "M"
...
# Defaults: additive error, no trend, no seasonality
model_default <- ETSModel()
fitted_default <- fit_baseline(data, model_default)

# Specific: additive error, additive trend, multiplicative season. A
# multiplicative error (error_type = "M") is available too; on this series it
# forecasts less accurately.
model_specific <- ETSModel(
  error_type = "A", trend_type = "A", season_type = "M", s = 12
)
...
ggplot(plot_df, aes(x = time, y = value, color = type)) +
  ...
  scale_color_manual(values = c("Observed" = "black", "Forecast" = "red")) +
  labs(
    title = "ETS Model: Complex Seasonality", x = "Time", y = "Value",
    color = NULL
  ) +
  theme_minimal() +
  theme(legend.position = "top")
...
- Start from the (A,N,N) defaults and add components the data calls for
