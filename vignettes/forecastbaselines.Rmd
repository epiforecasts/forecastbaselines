...
plot_df <- rbind(
  data.frame(time = 1:n, value = data, type = "Observed"),
  data.frame(time = (n + 1):(n + 12), value = fc$mean, type = "Constant"),
  data.frame(time = (n + 1):(n + 12), value = arma_fc$mean, type = "ARMA")
)

ggplot(plot_df, aes(x = time, y = value, color = type)) +
  geom_line(linewidth = 1) +
  scale_color_manual(values = c("Observed" = "black", "Constant" = "red", "ARMA" = "blue")) +
  labs(title = "Constant vs ARMA Forecasts", x = "Time", y = "Value", color = NULL) +
  theme_minimal() +
  theme(legend.position = "top")
```
