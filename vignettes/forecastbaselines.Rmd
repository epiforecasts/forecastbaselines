...
# Plot the data
ggplot(data.frame(time = 1:n, value = data), aes(x = time, y = value)) +
  geom_line() +
...
plot_df <- rbind(
  data.frame(time = 1:n, value = data, type = "Observed"),
  data.frame(time = (n + 1):(n + 12), value = fc$mean, type = "Forecast")
)
...
# Add truth for visualization
truth_vals <- c(105, 110, 108, 112, 115, 118, 120, 122, 125, 128, 130, 132)
fc_intervals_truth <- add_truth(fc_with_intervals, truth_vals)

# Convert to scoringutils format and visualize
fc_quantile <- scoringutils::as_forecast_quantile(fc_intervals_truth)
...
# Add truth and visualize with prediction intervals
truth <- c(105, 110, 108, 112, 115, 118, 120, 122, 125, 128, 130, 132)
arma_with_truth <- add_truth(arma_fc, truth)

# Convert to scoringutils format and plot with intervals
arma_quantile <- scoringutils::as_forecast_quantile(arma_with_truth)

# Show quantile forecast structure
print(head(arma_quantile, 15))
```

## Visualizing with hubVis

The package integrates with hubVis for interactive forecast visualization:

```{r hubvis, eval = FALSE}
# Convert to hubverse format
hubverse_data <- as_hubverse(
  arma_with_truth,
  origin_date = as.Date("2024-01-01"),
  horizon_unit = "week"
)
...
if (requireNamespace("hubVis", quietly = TRUE) &&
  ...
  # Convert to model_out_tbl format
  model_out <- hubUtils::as_model_out_tbl(hubverse_data$model_output)

  # Create interactive plot with 50%, 80%, and 95% intervals
  hubVis::plot_step_ahead_model_output(
    model_out,
    hubverse_data$target_data,
    use_plotly = TRUE # Set FALSE for static ggplot
  )
}
```

This creates an interactive Plotly visualization showing:
- Prediction intervals at 50%, 80%, and 95% confidence levels
- Observed values overlaid on the forecasts
...
# Compare
n_seasonal <- length(seasonal_data)
plot_df <- rbind(
  data.frame(time = 1:n_seasonal, value = seasonal_data, type = "Data"),
  data.frame(time = (n_seasonal + 1):(n_seasonal + 12), value = stl_fc$mean, type = "STL"),
  data.frame(time = (n_seasonal + 1):(n_seasonal + 12), value = lsd_fc$mean, type = "LSD")
)

ggplot(plot_df, aes(x = time, y = value, color = type)) +
  geom_line(linewidth = 1) +
  scale_color_manual(values = c("Data" = "black", "STL" = "red", "LSD" = "blue")) +
  labs(title = "Seasonal Forecasting", x = "Time", y = "Value", color = NULL) +
  theme_minimal() +
  theme(legend.position = "top")
```
